    ...
    public int minimumOperations(int[] nums) {
        ...
        for (int key : odd.keySet()) {
            if (even.getOrDefault(key,0) != 0) {
                if (even.get(key) > odd.get(key)) odd.put(key, 0);
                else even.put(key, 0);
            }
        }
        int oddNum = 0;
        int evenNum = 0;
        for (int key : odd.keySet()) oddNum = Math.max(oddNum, odd.get(key));
        for (int key : even.keySet()) evenNum = Math.max(evenNum, even.get(key));
